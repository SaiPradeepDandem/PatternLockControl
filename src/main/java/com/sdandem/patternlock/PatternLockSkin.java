...
public class PatternLockSkin extends SkinBase<PatternLockControl, PatternLockBehavior> {
	...
	class Dot extends StackPane {
		...
		public Dot(final int number, final int row, final int col) {
			super();
			getStyleClass().add("dot");
			outer = new Circle();
			outer.getStyleClass().add("outer-circle");

			inner = new Circle(5);
			inner.getStyleClass().add("inner-circle");
			...
			active.addListener(new ChangeListener<Boolean>() {
				@Override
				public void changed(ObservableValue<? extends Boolean> paramObservableValue, Boolean paramT1, Boolean active) {
					outer.getStyleClass().removeAll("outer-circle-active", "outer-circle-invalid");
					inner.getStyleClass().removeAll("inner-circle-active", "inner-circle-invalid");
					if (active) {
						outer.getStyleClass().add("outer-circle-active");
						inner.getStyleClass().add("inner-circle-active");
					}
				}
			});
			...
			invalid.addListener(new ChangeListener<Boolean>() {
				@Override
				public void changed(ObservableValue<? extends Boolean> paramObservableValue, Boolean paramT1, Boolean invalid) {
					outer.getStyleClass().removeAll("outer-circle-active", "outer-circle-invalid");
					inner.getStyleClass().removeAll("inner-circle-active", "outer-circle-invalid");
					if (isActive() && invalid) {
						outer.getStyleClass().add("outer-circle-invalid");
						inner.getStyleClass().add("inner-circle-invalid");
					}
				}
			});
		}
